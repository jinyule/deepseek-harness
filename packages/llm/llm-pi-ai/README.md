...
API keys never enter that collection. The harness resolves a route's named key through its own seam before the request reaches pi-ai and passes it as the highest-priority request override, preserving fail-loud reference semantics. When `@deepseek-ai/dsh-llm-pi-ai-oauth` is mounted, the same snapshot receives its durable pi-ai `CredentialStore`; a route naming no key can then use provider-native OAuth, while providers without a stored OAuth credential retain their own ambient-auth behavior.
...
- **OAuth-only providers depend on the companion service** — `@deepseek-ai/dsh-llm-pi-ai-oauth` owns interactive login, owner-only persistence, and serialized refresh. OAuth-only catalog routes such as `openai-codex` appear in the configurable-provider directory while that service is mounted; API-key-only compositions continue to omit them. A settings document that already names one keeps its entry so it remains editable.
