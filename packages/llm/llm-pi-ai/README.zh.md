...
API 密钥绝不进入该集合。harness 在请求抵达 pi-ai 之前经自身 seam 解析路由点名的密钥，并作为优先级最高的请求覆盖传入，从而保住引用缺失时明确失败的语义。挂载 `@deepseek-ai/dsh-llm-pi-ai-oauth` 后，同一快照还会接收其持久化 pi-ai `CredentialStore`；未点名密钥的路由便可使用提供方原生 OAuth，而没有已存 OAuth 凭据的提供方仍按自己的环境认证方式工作。
...
- **仅以 OAuth 认证的提供方依赖配套服务**：`@deepseek-ai/dsh-llm-pi-ai-oauth` 负责交互式登录、仅属主可读的持久化与串行刷新。挂载该服务时，`openai-codex` 等仅 OAuth catalog 路由会进入可配置提供方目录；仅 API 密钥的组合仍不列出它们。settings 文档已经写过的路由仍保留目录条目，因此始终可以编辑。
