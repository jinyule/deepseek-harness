...
提供方原生 OAuth 是来自 `ctx.remote.piAiOAuth` 的第四项可选补充信息。仅 OAuth 行只有在该服务确认持久化凭据存在后才算可用；其编辑器会用提供方自带的登录选项、浏览器或设备码指引、精确 prompt 回答、取消与退出取代 API 密钥字段。登录事件不携带答案或 token；登录、退出、刷新或流程生命周期变化后，`pi-ai-oauth/updated` 会重新读取共享联接。双认证提供方会在 OAuth 旁保留 API 密钥字段。删除提供方 profile 不会删除 OAuth 账号凭据；该操作只属于明确的「退出登录」。
...
- **OAuth 控件跟随已安装提供方的提示**：页面渲染 pi-ai 转发的 prompt 类型与链接，不自行发明提供方专属恢复或账号管理流程。
