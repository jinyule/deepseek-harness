...
| `ctx.piAiOAuth` | `core` | [`llm-pi-ai-oauth`](../packages/llm/llm-pi-ai-oauth) | - | [`llm-pi-ai`](../packages/llm/llm-pi-ai), `apiproxy` | - | 拥有持久化 pi-ai 凭据存储、串行化刷新与交互式登录；`llm-pi-ai` 把同一个存储注入提供方集合，Host API 则转发不含实际值的登录事件。 |
